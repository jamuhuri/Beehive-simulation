...
public class Forager extends Ant{
    ...
    public ColonyNode findMostPheromone() {
        ...
        if (adjacentList.size() == 0) {
            adjacentList = antLocation.getAdjacentNodes();
        }
    ...
    public void move(ColonyNode node) {
        ...
        if (antLocation.hasQueen() && returnToNest) {
            antLocation.setFood(antLocation.getFood() + 1);
            returnToNest = false;
            moveHistory.clear();
        }
        moveHistory.add(antLocation);
    ...
    public boolean foundFood() {
        if (antLocation.getFood() > 0 && !(antLocation.hasQueen())) {
            antLocation.setFood(antLocation.getFood() - 1);
            returnToNest = true;
    ...
    public void depositPheromones() {
        if (!(antLocation.hasQueen())) {
            if (antLocation.getPheromone() < 1000) {
                antLocation.setPheromone(antLocation.getPheromone() + 10);
            }
        }
    }
